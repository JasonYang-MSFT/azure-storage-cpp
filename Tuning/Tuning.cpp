...
public:
	...
	int single_download_test_memory_allocated(size_t length, int parallelism);
	...
	utility::string_t download_test_memory_allocated()
	{
		utility::stringstream_t out;
		std::vector<size_t> lens{ 3, 30, 100, 1000 };
		for (size_t index = 0; index < lens.size(); ++index)
		{
			...
			for (int i = 1; i <= parallel; ++i)
			{
				ucout << length << L" " << i << " " << single_download_test_memory_allocated(length, i) << std::endl;
			}
		}
		return out.str();
	}
...
int Performance::single_download_test_memory_allocated(size_t length, int parallelism)
{
	azure::storage::cloud_blob_container container = client.get_container_reference(L"perftests");
	container.create_if_not_exists();
	azure::storage::cloud_append_blob blob = container.get_append_blob_reference(utility::conversions::print_string(length));
	...
	for (int i = 0; i < times; ++i)
	{
		...
		option.set_parallelism_factor(parallelism);
		concurrency::streams::container_buffer<std::vector<uint8_t>> download_buffer;
		download_buffer.alloc(length * 1024 * 1024);
		auto start = clock();
		blob.download_to_stream(download_buffer.create_ostream(), azure::storage::access_condition(), option, azure::storage::operation_context());
		auto end = clock();
		total += end - start;
...
int main()
{
	...
	// ucout << instance.single_download_test_memory_allocated(1000, 5) << std::endl;
	// ucout << instance.single_download_test_filestream(1000, 5) <<std::endl;
	ucout << instance.download_test() << std::endl;
	ucout << L"*******************************************" << std::endl;
	ucout << instance.download_test_memory_allocated() << std::endl;
    ucout << L"*******************************************" << std::endl;
